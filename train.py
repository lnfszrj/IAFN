...
class Instructor:
    def __init__(self, opt):
        ...
        absa_dataset = ABSADatesetReader(transform, dataset=opt.dataset, embed_dim=opt.embed_dim,
        ...
        net.load_state_dict(torch.load(opt.resnet_root))
    ...
    def run(self):
        ...
        if self.opt.load_check_point:
            if os.path.isfile(
                    self.opt.checkpoint + self.opt.dataset + '_' + self.opt.model_name + '_' + self.opt.att_mode + '.model'):
        ...
        if self.opt.load_check_point:
            ...
            with torch.no_grad():
                ...
                if self.opt.att_mode == 'vis_att_img_gate' or self.opt.att_mode == 'vis_concat_attimg_gate':
                    ...
                    fout_l = open(
                        self.opt.att_file + os.sep + self.opt.dataset + '_' + self.opt.model_name + '_' + self.opt.att_mode + '_' + 'left_att.txt',
                        'w')
                    fout_r = open(
                        self.opt.att_file + os.sep + self.opt.dataset + '_' + self.opt.model_name + '_' + self.opt.att_mode + '_' + 'right_att.txt',
                        'w')
                    fout = open(
                        self.opt.att_file + os.sep + self.opt.dataset + '_' + self.opt.model_name + '_' + self.opt.att_mode + '_' + 'vis_att.txt',
                        'w')

                if not os.path.exists(self.opt.pred_file):
                    os.mkdir(self.opt.pred_file)
                fout_p = open(
                    self.opt.pred_file + os.sep + self.opt.dataset + '_' + self.opt.model_name + '_' + self.opt.att_mode + '_' + 'pred.txt',
                    'w')
                fout_t = open(
                    self.opt.pred_file + os.sep + self.opt.dataset + '_' + self.opt.model_name + '_' + self.opt.att_mode + '_' + 'true.txt',
                    'w')
                ...
                for t_batch, t_sample_batched in enumerate(self.test_data_loader):
                    ...
                    if self.opt.att_mode != 'vis_att_img_gate' and self.opt.att_mode != 'vis_concat_attimg_gate':
                        ...
                        t_outputs, l_att, r_att, att_weights = self.model(t_inputs, t_imgs_f, t_img_mean, t_img_att,
                                                                          self.opt.att_mode)
                        t_preds = torch.argmax(t_outputs, -1)
            ...
            for epoch in range(self.opt.num_epoch):
                ...
                for i_batch, sample_batched in enumerate(self.train_data_loader):
                    ...
                    img_mean = torch.Tensor([]).to(device)
                    img_mean2 = torch.Tensor([]).to(device)
                    img_mean3 = torch.Tensor([]).to(device)

                    img_att = torch.Tensor([]).to(device)
                    img_att2 = torch.Tensor([]).to(device)
                    img_att3 = torch.Tensor([]).to(device)

                    imgs_f = torch.Tensor([]).to(device)
                    imgs_f2 = torch.Tensor([]).to(device)
                    imgs_f3 = torch.Tensor([]).to(device)

                    images1 = sample_batched['image1'].to(device)
                    images2 = sample_batched['image2'].to(device)
                    images3 = sample_batched['image3'].to(device)
                    # print(images1.shape)
                    # print(images2.shape)
                    with torch.no_grad():
                        if images1.shape[1] != 0:
                            imgs_f, img_mean, img_att = self.encoder(images1)
                        if images2.shape[1] != 0:
                            imgs_f2, img_mean2, img_att2 = self.encoder(images2)
                            # print("imgs_f2.device:", imgs_f2.device)
                        if images3.shape[1] != 0:
                            imgs_f3, img_mean3, img_att3 = self.encoder(images3)

                    if self.opt.att_mode != 'vis_att_img_gate' and self.opt.att_mode != 'vis_concat_attimg_gate':
                        outputs = self.model(inputs, imgs_f, img_mean, img_att, imgs_f2, img_mean2, img_att2, imgs_f3,
                                             img_mean3, img_att3, self.opt.att_mode)
                    else:
                        outputs, _, _, _ = self.model(inputs, imgs_f, img_mean, img_att, imgs_f2, img_mean2, img_att2,
                                                      imgs_f3, img_mean3, img_att3, self.opt.att_mode)
                    ...
                    if global_step % self.opt.log_step == 0:
                        ...
                        with torch.no_grad():
                            for t_batch, t_sample_batched in enumerate(self.train_data_loader):
                                ...
                                t_img_mean = torch.Tensor([]).to(device)
                                t_img_mean2 = torch.Tensor([]).to(device)
                                t_img_mean3 = torch.Tensor([]).to(device)

                                t_img_att = torch.Tensor([]).to(device)
                                t_img_att2 = torch.Tensor([]).to(device)
                                t_img_att3 = torch.Tensor([]).to(device)

                                t_imgs_f = torch.Tensor([]).to(device)
                                t_imgs_f2 = torch.Tensor([]).to(device)
                                t_imgs_f3 = torch.Tensor([]).to(device)

                                t_images = t_sample_batched['image1'].to(device)  # 需更改
                                t_images2 = t_sample_batched['image2'].to(device)
                                t_images3 = t_sample_batched['image3'].to(device)

                                if t_images.shape[1] != 0:
                                    t_imgs_f, t_img_mean, t_img_att = self.encoder(t_images)
                                if t_images2.shape[1] != 0:
                                    t_imgs_f2, t_img_mean2, t_img_att2 = self.encoder(t_images2)
                                if t_images3.shape[1] != 0:
                                    t_imgs_f3, t_img_mean3, t_img_att3 = self.encoder(t_images3)

                                if self.opt.att_mode != 'vis_att_img_gate' and self.opt.att_mode != 'vis_concat_attimg_gate':
                                    t_outputs = self.model(t_inputs, t_imgs_f, t_img_mean, t_img_att, self.opt.att_mode)
                                else:
                                    t_outputs, _, _, _ = self.model(t_inputs, t_imgs_f, t_img_mean, t_img_att,
                                                                    t_imgs_f2, t_img_mean2, t_img_att2,
                                                                    t_imgs_f3, t_img_mean3, t_img_att3,
                                                                    self.opt.att_mode)
                            ...
                            for t_batch, t_sample_batched in enumerate(self.dev_data_loader):
                                ...
                                t_images = t_sample_batched['image1'].to(device)
                                t_images2 = t_sample_batched['image2'].to(device)
                                t_images3 = t_sample_batched['image3'].to(device)
                            ...
                            if dev_acc > max_dev_acc:
                                ...
                                track_list.append(
                                    {'loss': loss.item(), 'dev_acc': dev_acc, 'dev_p': dev_p, 'dev_r': dev_r,
                                     'dev_f1': dev_f1, \
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default='twitter2017', type=str, help='twitter, snap')
    ...
    # parser.add_argument('--path_image', default='../../../visual_attention_ner/twitter_subimages',
    # help='path to images')
    # /home/xinke901/zrj/zrj/TASL2021/TASL2020/ESAFN-master/IJCAIdata/twitter2017_images
    # /home/xinke901/zrj/zrj/TASL2021/TASL2020/ESAFN-master/IJCAIdata/twitter2015_images
    parser.add_argument('--path_image',
                        default='/home/xinke901/zrj/zrj/TASL2021/TASL2020/ESAFN-master/IJCAIdata/twitter2017_images',
                        help='path to images')
    # parser.add_argument('--path_image', default='./twitter15_images',
    # help='path to images')
    # parser.add_argument('--path_image', default='./snap_subimages',
    # help='path to images')
    ...
    parser.add_argument('--att_mode', choices=['text', 'vis_only', 'vis_concat', 'vis_att', 'vis_concat_attimg', \
                                               ...
                                               'vis_att_img_gate', 'vis_concat_attimg_gate'],
                        default='vis_concat_attimg_gate', \
                        help='different attention mechanism')
    parser.add_argument('--resnet_root', default='./resnet.pth', help='path the pre-trained cnn models')
    ...
    if opt.dataset == "twitter":
        ...
        opt.path_image = "/home/xinke901/zrj/zrj/TASL2021/TASL2020/ESAFN-master/IJCAIdata/twitter2015_images"
        # opt.path_image = "../multi_modal_ABSA_pytorch_bilinear/twitter15_images/"
        opt.max_seq_len = 24
        opt.rand_seed = 25
    elif opt.dataset == "twitter2017":
        opt.path_image = "/home/xinke901/zrj/zrj/TASL2021/TASL2020/ESAFN-master/IJCAIdata/twitter2017_images"
        # opt.path_image = "../multi_modal_ABSA_pytorch_bilinear/twitter17_images/"
        opt.max_seq_len = 24
        opt.rand_seed = 25
    else:
