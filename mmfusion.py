...
class MMFUSION(nn.Module):
    ...
    def forward(self, inputs, visual_embeds_global, visual_embeds_mean, visual_embeds_att, att_mode):
        ...
        avg_aspect = torch.div(sum_aspect, aspect_len.view(aspect_len.size(0), 1))          # 平均池化后的方面词特征向量
        ...
        if att_mode == 'text': # apply aspect words to attend the left and right contexts
            ...
            if self.opt.tfn:
                ...
                # 
                # person_trait = self.trait_kernel(avg_aspect, l_context, r_context, trait_class_index)
                # x = torch.cat((x, person_trait),dim = -1)
